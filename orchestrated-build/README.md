...
This skill executes a written implementation plan with a fleet of agents: one Opus 5 low subagent per vertical slice, each spawning Luna Max as a native Codex subagent through the Codex plugin, then one review phase and one end-to-end phase. An agent triggers it when a user asks to execute or ship a plan, orchestrate or parallelize agents, or run long autonomous builds without losing them when a turn ends.
